...
const givePersonalFeedback = require('./bot activities/daily feedback/bot');
...
client.on('message', msg => {
	...
	if (
		...
		if (dateIsValid) {
				...
				.then(response => {
					...
					isConfirmedInDatabase(msg.author.username, msg.channel.name)
						.then(isCompleted => {
							if (isCompleted) {
								msg.author.send(
									`Wow, you did it! How wonderful. You have successfully completed the ${
										msg.channel.name
									} challenge. You should be proud. Congratulations.`
								);
							}
						})
						.catch(err => {
			...
			msg.reply(
				`Invalid day, 100 Days only. Command to restart this challenge is $restart-${
					msg.channel.name
				}`
			);
	}
...
const validateDay = msg => {
	const dayInMsg = msg.match(/(\$day\d+)/);

	const dayInNum = dayInMsg && dayInMsg[0].match(/(\d+)/)[0];

	if (dayInNum <= 0 || dayInNum > 100 || !dayInNum) {
		return false;
	}
...
// @bot-command::   $feedback-100daysofcode || #feedback-100daysoflearning
// @access: all channels of this bot
givePersonalFeedback('elijahtrillionz')
client.on('message', msg => {
	if (msg.author.bot) return;

  const msgInLowerCase = msg.content.toLowerCase()
	if (msgInLowerCase === `$feedback-${msg.channel.name}`) {
		givePersonalFeedback(msg.author.username, msg.channel.name)
			.then(res => {
				msg.author.send(res)
			})
			.catch(err => {
